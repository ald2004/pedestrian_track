            ...
            while 1:
                for conn in connected:
                    ...
                    for i in range(len(realbox)):
                        ...
                        # enhance movment
                        # for i in range(int((x1 + x2) / 2), int((x1 + x2) / 2) + np.random.randint(20), 5):
                        #     jsondump_enhance = [{"id": bname, "x": i, "y": int((y1 + y2) / 2 + np.random.randint(-10, 10)),
                        #                          "time": time.time()}]
